...
-   Update personal context (add family member)

**Schedule Management**:

-   Create daily medication reminder
-   Create weekly meal reminder
-   Create one-time reminder
-   Edit existing schedule
-   Delete schedule (verify soft delete)
...
-   Respond with voice
-   Verify response logged in dashboard
-   Test retry mechanism (don't respond)
-   Verify alert created after max retries

**Conversation Flow**:

-   Tap "Talk to Me" on mobile
-   Speak a message
...
-   Test various message types:
    ...
    -   Health concern
    -   Casual chat
    -   Family member mention

**AI Learning**:

-   Have 3+ interactions with similar pattern
-   Check Letta insights generation
-   Verify Claude uses Letta context in responses
-   Check daily summary for AI observations

**Emergency Flow**:

-   Press emergency button on mobile
-   Confirm action
-   Verify alert created
-   Verify SMS sent to caregiver
-   Verify voice call made to caregiver
-   Verify dashboard shows alert
-   Acknowledge alert in dashboard

**Activity Monitoring**:

-   Verify heartbeat sent every 15 minutes
-   Simulate 4 hours of inactivity
...
-   Check summary content (stats, narrative, insights)
-   Verify summary visible in dashboard

**Mobile App Offline**:

-   Disable phone's internet
...
-   Patient responds in unexpected way (gibberish)
-   Patient mentions multiple topics in one message
-   Multiple reminders at same time
-   Emergency button during ongoing conversation
-   Low battery during heartbeat
...
-   Heartbeat sent every 15 minutes (±1 minute)
-   Alerts sent within 5 seconds of trigger
...
-   All text is readable (large fonts)
-   All buttons are tappable (large touch targets)

**AI Quality**:

-   Claude correctly identifies intent >90% of time
-   Letta insights are relevant and accurate
-   Daily summary is coherent and useful
...
4.  Add Python service (for FastAPI)
...
9.  Test API endpoints (use production URL)
...
-   Update Twilio webhook URLs to point to Railway URL
...
-   No build needed
-   Install Expo Go app on demo device
-   Scan QR code to load app
-   Pros: Instant, no build process
-   Cons: Expo Go branding visible
...
-   Pros: Looks like real app
-   Cons: Takes time to build, may need Apple Developer account for iOS

**For Hackathon**: Use Option A (Expo Go) unless team has time and Apple Developer account.

**Steps for Expo Go**:

1.  Install Expo Go on demo device
2.  Run `npx expo start` from mobile project
3.  Scan QR code with device
4.  Test app thoroughly on device

**Steps for EAS Build** (if time permits):
...
-   TTS works
-   Push notifications work (Firebase configured)
-   Heartbeat sending

**Integration**:

-   End-to-end flow works in production
-   Dashboard shows data from mobile
-   Alerts trigger and notify
-   SMS/calls from Twilio work
...
-   Laptop for dashboard (presenter uses)
-   Phone with mobile app (patient simulation)
-   Backup phone (if first one fails)
...
-   Pitch deck (Google Slides or PowerPoint)
-   Postman/Thunder Client (backup to show API)

**Network**:

-   Test venue WiFi beforehand
-   Have mobile hotspot as backup
-   Know WiFi credentials
-   Test that mobile app works on venue WiFi
...
-   Hook: "My grandmother has Alzheimer's. Last month, she forgot her insulin twice and ended up in the hospital."
-   Statistics: "65 million elderly Americans live with memory issues. 270,000 die yearly from medication errors."
-   Existing solutions: "Current solutions are just alarms - impersonal, annoying, and patients ignore them."

**Minute 1-2: Solution Overview**

-   Introduce: "Elder Companion AI - a voice-first AI that learns, remembers, and cares."
-   Show system diagram
-   Key features: "It's not just reminders - it's a companion that adapts to each person."

**Minute 2-4: Live Demo** _Switch to dashboard on laptop_

-   "This is Maggie, a 78-year-old with mild dementia."
-   "Here's her schedule for today." (point to dashboard)
-   "Watch what happens when it's time for her medication."

_Pick up phone with mobile app_

-   Trigger reminder (or wait for scheduled one if timing works)
-   Phone receives notification, TTS plays: "Hi Maggie, time for your yellow pill on the kitchen table."
-   Speak response: "I already took it."

_Back to dashboard_

-   "See how it logged her response immediately?"
-   "The AI understands she completed the task."
-   Show Claude's analysis: "It knows she's reliable about morning pills."

_Back to mobile app_

-   Tap "Talk to Me" button
-   Speak: "I'm feeling dizzy."
-   AI responds with concern, asks follow-up questions

_Back to dashboard_

-   "The AI detected a potential health concern and alerted the caregiver."
-   Show alert in dashboard
-   "It also learned this is a recurring pattern - usually related to dehydration."

_Show daily summary_

-   "At the end of each day, AI generates a comprehensive report."
-   Point out: statistics, narrative summary, insights, recommendations

**Minute 4-5: Technology & Impact**

-   "We use Claude for real-time conversation understanding."
-   "Letta provides long-term memory - the AI learns each patient's patterns."
-   "Chroma enables semantic search through all past interactions."
-   Impact: "This prevents hospital visits, maintains independence, and reduces caregiver stress."
-   "Our target users: 65 million elderly Americans, their families, and care facilities."
-   Prizes: "We're targeting Social Impact, Best Use of Claude, Best Use of Letta, and AI with Chroma."
-   Future: "Next steps: IoT integration with Alexa and smart pill dispensers, partnerships with senior living facilities, and clinical trials."

**Close**: "Elder Companion AI - because everyone deserves to age with dignity and independence. Thank you!"
...
-   Use mobile hotspot
-   Demo API responses in Postman (offline mode)

**If Mobile App Crashes**:

-   Use backup phone
-   Show video of mobile app
-   Focus demo on dashboard features
-   Explain mobile app verbally with screenshots

**If Backend Down**:

-   Show demo video
-   Show Postman collection with sample responses
-   Walk through code briefly
-   Explain architecture with diagrams

**If Voice Recognition Fails**:

-   Type the message instead (show it works without voice)
-   Blame venue noise
-   Focus on AI intelligence, not voice interface
...
**Q: How does the AI actually learn?** A: "We use Letta, which maintains a long-term memory for each patient. After every interaction, we send a summary to Letta - what happened, how the patient responded, outcomes. Letta builds a model of this specific patient's patterns over time. Then, before Claude generates any response, we query Letta for context like 'what communication style works best?' or 'has the patient mentioned this before?' This makes every interaction personalized."

**Q: What about privacy and HIPAA?** A: "Great question. For this hackathon MVP, we focused on core functionality. For production, we'd implement full HIPAA compliance: encrypted data at rest and in transit, audit logs, Business Associate Agreements with all vendors, and patient consent forms. All our vendors (Anthropic, Letta) offer HIPAA-compliant tiers."

**Q: How is this different from existing medical alert systems?** A: "Three key differences: First, we're conversational, not just alarms. Patients can talk to us anytime. Second, we learn and adapt - if Grandma responds better to reminders at 8:30 instead of 8:00, we learn that. Third, we provide caregivers with deep insights, not just 'alert: no response.' We tell them WHY and provide recommendations."

**Q: Can it integrate with IoT devices like Alexa?** A: "Absolutely! The architecture is designed for this. Our backend API can talk to any device. We focused on mobile for the hackathon because it's universal - everyone has a phone. But adding Alexa, Google Home, or smart pill dispensers is just a matter of adding those integration endpoints. That's our immediate next step post-hackathon."

**Q: What's the business model?** A: "We see two main revenue streams: First, direct-to-consumer at $20-50/month - that's less than one day of nursing home care. Second, B2B partnerships with senior living facilities and insurance companies. Insurance companies have huge incentive to prevent hospitalizations - one prevented ER visit pays for years of our service. We'd also explore value-based care models where we get paid based on outcomes."

**Q: How accurate is the voice recognition for elderly users?** A: "We use the device's native speech recognition, which is pretty good for elderly voices. We've also built in retry mechanisms - if the AI doesn't understand, it asks for clarification in simpler terms. Additionally, Letta learns each patient's speech patterns over time, so accuracy improves. For users with severe speech difficulties, we also support typed input via the caregiver dashboard."

**Q: What if the patient doesn't have a smartphone?** A: "Great point! That's where our IoT roadmap comes in - Alexa, Google Home, even landline phone integration. For the hackathon, we focused on mobile because of time constraints, but the backend architecture supports any interface. We'd also partner with senior centers to provide subsidized devices."

**Q: How do you handle emergencies?** A: "Multiple layers: First, the giant red emergency button triggers immediate alerts to caregivers via SMS and voice call. Second, our AI detects distress in conversation - phrases like 'I fell' or 'chest pain' trigger high-priority alerts. Third, passive monitoring - if no activity for 4+ hours during daytime, we alert caregivers for a welfare check. We don't auto-dial 911 because we want caregivers to assess first, but it's one button press away."
...
-   Clear problem: Elderly care crisis
-   Large affected population: 65M+ Americans
-   Measurable impact: Prevented hospitalizations, maintained independence
-   Emotional appeal: Personal stories about grandparents

**Best Use of Claude**:

-   Advanced use case: Conversational AI with context awareness
-   Multiple Claude applications: Message understanding, response generation, daily summaries
-   Sophisticated prompting: Context-aware, personalized
-   Meaningful integration: Claude is core to solution, not superficial

**Best Use of Letta**:

-   Perfect use case for stateful agents: Long-term memory for each patient
-   Clear learning over time: Adapts reminders, communication style
...
-   Mobile app working on demo device
-   All APIs tested in production
-   Demo data loaded (test patient, schedules, conversations)
-   Battery charged on all devices
-   Backup video uploaded and link ready

**Presentation**:

-   Pitch deck finalized and accessible
...
-   Devpost submission finished
-   Screenshots taken
-   Demo video uploaded
-   Prize justifications written

**Logistics**:

-   Know judging location and time
...
-   ✅ Mobile app can receive reminder and respond
-   ✅ Dashboard shows real-time data
-   ✅ AI (Claude) understands at least one response correctly
-   ✅ Demo doesn't crash during presentation
...
-   ✅ Emotional demo that resonates with judges
-   ✅ Professional polish (UI, UX, presentation)
-   ✅ Clear technical sophistication
-   ✅ Win 2-3 prizes

### Outstanding Success (Multiple Prize Winner)

-   ✅ Everything above PLUS
-   ✅ Letta insights are impressive and useful
-   ✅ Chroma semantic search demonstrates value
-   ✅ Voice interaction is natural and reliable
-   ✅ Judges ask "when can I use this for my parent?"
...
-   Every feature you build could save someone's life
-   Your grandparents/parents will use this someday
-   This is more than a hackathon project - it's meaningful work

**If things go wrong**:

-   Perfect is the enemy of done
-   Working demo of ONE feature > broken implementation of ten features
-   Judges care about problem-solution fit, not perfection
-   Personal connection > technical perfection

**You've got this because**:

-   The problem is clear and relatable
-   The solution is technically impressive yet achievable
-   The AI integration is meaningful, not superficial
-   You have 20 hours to build something that matters

**Final thought**: "In 20 hours, you're going to build technology that could prevent someone's grandmother from ending up in the hospital. That's worth fighting for. Now go build it!" 🚀

----------

**END OF SPECIFICATION**

This is the complete, detailed project specification. Hand this to Claude Code and say: "Build Elder Companion AI following this specification. Start with Hour 0-2 tasks."